  ...
  for (auto const repr : p.value()->Representation_.value()->Representations_) {
    for (auto const item : repr->Items_) {
      ...
      auto mapped_repr = mapped_item->MappingSource_
                             ->MappedRepresentation_;  // IfcRepresentation
      auto const mapping_origin =
          mapped_item->MappingSource_->MappingOrigin_;  // axis2placement
  ...
  for (auto const repr :
    ...
    for (auto const item : repr->Items_) {
      ...
      auto mapped_repr = mapped_item->MappingSource_
                             ->MappedRepresentation_;  // IfcRepresentation
      auto pds =
          dynamic_cast<IFC2X3::IfcProductDefinitionShape const*>(mapped_repr);
      auto src_vertices = ifcgeom::gather_vertices(pds);

      // ------------------------------------------------------------------
      // MappingTarget 	 :  	IfcCartesianTransformationOperator;
      // ------------------------------------------------------------------
      auto const mapping_transf = ifcgeom::cartesian_transformation_non_uniform(
          dynamic_cast<
              IFC2X3::IfcCartesianTransformationOperator3DnonUniform const*>(
              mapped_item->MappingTarget_));
    }
  }
}
